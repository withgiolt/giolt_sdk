...
get_os() ->
    case erlang:system_info(os_type) of
        ...
        {unix, sunos} ->
            {ok, solaris};
        {unix, sunos4} ->
            {ok, sunos}; % technically not solaris
        {unix, freebsd} ->
            {ok, freebsd};
        {unix, openbsd} ->
            {ok, openbsd};
...
do_fetch(Url) ->
    ...
    case get_os() of
        {win32, _} ->
            {ok =
                 erl_tar:extract({binary, Binary},
                                 [compressed,
                                  verbose,
                                  {cwd, install_dir()},
                                  {files, ["package/bin/esbuild.exe"]}])};
        _ ->
            {ok =
                 erl_tar:extract({binary, Binary},
                                 [compressed,
                                  verbose,
                                  {cwd, install_dir()},
                                  {files, ["package/bin/esbuild"]}])}
    end.

-spec do_exec(string(), [string()]) -> a.
do_exec(Cmd, Args) ->
    % Exec = case get_os() of
    ...
    do_receive(Port).    % receive {Port, {exit_status, Code}} -> Code end
                         % get_port_data(Port)
