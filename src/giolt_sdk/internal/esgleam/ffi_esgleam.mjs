...
export function exec_shell(command) {
  ...
  let res = spawnSync(command[0], command.slice(1), {
    cwd: ".",
    stdio: "inherit",
  });
...
export async function do_fetch(tarball_url, then) {
  ...
  for await (const entry of entries(tarStream)) {
    if (/(^|\/)esbuild(\.exe)?$/.test(entry.name)) {
      ...
      const exePath = entry.name.endsWith(".exe")
        ? `${pkg_dir}/esbuild.exe`
        : `${pkg_dir}/esbuild`;
      await writeFile(exePath, Buffer.from(await entry.arrayBuffer()), {
        encoding: "binary",
      });
...
export function install() {
  spawnSync("gleam", ["run", "--target=erlang", "-m", "esgleam/install"], {
    stdio: "inherit",
  });
}
